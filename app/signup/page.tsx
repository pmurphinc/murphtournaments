...
async function upsertTeam(formData: FormData, headers: Headers) {
  "use server";
  ...
  const captainUser = await prisma.user.findUnique({ where: { discordId: (session.user as any).discordId } });
  if (!captainUser) throw new Error("User not found");

  const team = await prisma.team.upsert({
    where: { name: parsed.data.teamName },
    update: { tournamentId: parsed.data.tournamentId, notes: `${parsed.data.platform}/${parsed.data.region}` },
    create: {
      ...
      members: { create: [
        ...
        ...(parsed.data.embarkSub ? [{ displayName: "Sub", embarkId: parsed.data.embarkSub, isSub: true }] : [])
      ] }
    }
  });
...
export default async function SignUpPage() {
  const session = await auth();
  const tournaments = await prisma.tournament.findMany({ orderBy: { startsAt: "desc" } });
  return (
    <div className="max-w-2xl space-y-6">
      <h1 className="text-2xl">Team Sign-Up</h1>
      {!session?.user && <p className="opacity-80">Please sign in with Discord to continue.</p>}
      <form action={upsertTeam} className="space-y-4" aria-describedby="signup-help">
        <p id="signup-help" className="sr-only">All fields required. Embark IDs for 3 starters, optional sub.</p>
        <label htmlFor="tournamentId" className="sr-only">Tournament</label>
        <select id="tournamentId" name="tournamentId" className="w-full h-9 rounded-md border border-zinc-700 bg-black/50 px-2">
          {tournaments.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
        </select>
        <div><Label htmlFor="teamName">Team name</Label><Input id="teamName" name="teamName" minLength={3} required /></div>
        <div><Label htmlFor="captainDiscord">Captain Discord</Label><Input id="captainDiscord" name="captainDiscord" defaultValue={(session?.user as any)?.discordId ?? ""} required /></div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div><Label htmlFor="embark1">Embark ID 1</Label><Input id="embark1" name="embark1" required /></div>
          <div><Label htmlFor="embark2">Embark ID 2</Label><Input id="embark2" name="embark2" required /></div>
          <div><Label htmlFor="embark3">Embark ID 3</Label><Input id="embark3" name="embark3" required /></div>
          <div><Label htmlFor="embarkSub">Embark ID (Sub)</Label><Input id="embarkSub" name="embarkSub" /></div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label htmlFor="platform">Platform</Label>
            <select id="platform" name="platform" className="w-full h-9 rounded-md border border-zinc-700 bg-black/50 px-2">
              <option>PC</option><option>Xbox</option><option>PlayStation</option>
            </select>
          </div>
          <div>
            <Label htmlFor="region">Region</Label>
            <select id="region" name="region" className="w-full h-9 rounded-md border border-zinc-700 bg-black/50 px-2">
              <option>NA</option><option>EU</option><option>APAC</option>
            </select>
          </div>
        </div>
        ...
        <Button type="submit" className="w-full" aria-live="polite">Submit</Button>
      </form>
    </div>
  );
}